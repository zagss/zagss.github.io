...
(function () {
  var initial = {
    ...
    eventBind: function eventBind() {
      document.getElementById('submitBtn').onclick = this.handleSubmitClick;
    },
    handleSubmitClick: function handleSubmitClick(e) {
      var xingming = document.getElementById('user_name').value;
      var dianhua = document.getElementById('user_tel').value;
      ...
      if (!/^1(3|4|5|7|8)\d{9}$/.test(dianhua)) {
        return layer.open({
          content: '手机号码格式不正确!',
          skin: 'msg',
          time: 1
      ...
      console.log(xingming, dianhua); // var url = location.origin + '/plus/diyform2.php'

      _axios.default.post('/plus/diyform2.php', {
        xingming: xingming,
        dianhua: dianhua
      }).then(function (response) {
        layer.open({
          content: '提交成功，我们会第一时间与您联系!',
          skin: 'msg',
          time: 2
        });
      }).catch(function (error) {
        layer.open({
          content: '提交失败，请检查信息是否正确!',
          skin: 'msg',
          time: 2
        });
      });
